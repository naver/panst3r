...
"""Computing GT <-> mask matching for supervision."""
import torch
...
class HungarianMatcher(nn.Module):
    ...
    def memory_efficient_forward(self, outputs, targets):
        ...
        for b in range(bs):
            ...
            cost_mask = 0
            cost_dice = 0
            # Compute mask cost across all views
            ...
            assert out_mask.shape[1] == tgt_mask.shape[1], "Outputs and GT don't have same number of views"
            n_out, num_views = out_mask.shape[:2]
            n_tgt = tgt_mask.shape[0]

            # Concatenate views into batch dim
            tgt_mask = tgt_mask.flatten(0, 1).unsqueeze(1)
            out_mask = out_mask.flatten(0, 1).unsqueeze(1)

            # all masks share the same set of points for efficient matching!
            point_coords = torch.rand(num_views, self.num_points, 2, device=out_mask.device)
            ...
            tgt_mask = tgt_mask.unflatten(0, (n_tgt, num_views)).flatten(-2)  # NVxP -> NxVP
            ...
            out_mask = out_mask.unflatten(0, (n_out, num_views)).flatten(-2)  # NVxP -> NxVP
